...
/// 动画操作 : 需要UserID和回调
- (void)animWithUserID:(NSString *)userID model:(GiftModel *)model andViewY:(CGFloat)viewY finishedBlock:(void(^)(BOOL result))finishedBlock{
    ...
    if ([self.userGigtInfos objectForKey:userID]) {
        ...
        if ([userID integerValue] % 2) {
            ...
            if (op.model.giftCount != 0) {
                op.presentView.frame  = CGRectMake(-self.parentView.frame.size.width / 2, viewY+60, self.parentView.frame.size.width / 2, 50);
            ...
            if (op.model.giftCount != 0) {
                op.presentView.frame  = CGRectMake(-self.parentView.frame.size.width / 2, viewY, self.parentView.frame.size.width / 2, 50);
                op.presentView.originFrame = op.presentView.frame;
